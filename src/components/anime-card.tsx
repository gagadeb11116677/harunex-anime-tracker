"use client";

import * as React from "react";
import { Star, Check } from "lucide-react";
import type { Anime } from "@/lib/types";
import { useStore } from "@/lib/store";
import { cn, proxyImg } from "@/lib/utils";

interface Props {
  anime: Anime;
  className?: string;
  showRank?: boolean;
  rank?: number;
}

const PLACEHOLDER =
  "data:image/svg+xml;utf8," +
  encodeURIComponent(
    `<svg xmlns='http://www.w3.org/2000/svg' width='230' height='325'><rect width='100%' height='100%' fill='%23221a16'/><text x='50%' y='50%' fill='%23888080' font-family='sans-serif' font-size='12' text-anchor='middle' dominant-baseline='middle'>No image</text></svg>`
  );

export function AnimeCard({ anime, className, showRank, rank }: Props) {
  const selectAnime = useStore((s) => s.selectAnime);
  const items = useStore((s) => s.items);
  const [imgOk, setImgOk] = React.useState(true);
  const tracked = items[anime.mal_id];

  const img = proxyImg(
    anime.images?.webp?.large_image_url || anime.images?.jpg?.image_url
  ) || PLACEHOLDER;

  return (
    <button
      onClick={() => selectAnime(anime.mal_id)}
      className={cn(
        "group card-base pressable text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
        className
      )}
    >
      {/* Poster */}
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-muted">
        <img
          src={imgOk ? img : PLACEHOLDER}
          alt={anime.title}
          loading="lazy"
          onError={() => setImgOk(false)}
          className="poster-img size-full object-cover"
        />
        {/* top gradient + score pill */}
        <div className="pointer-events-none absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-black/55 to-transparent" />
        <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between p-2">
          {anime.score ? (
            <span className="pill-score inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-bold text-amber-300 ring-1 ring-white/10 backdrop-blur-md">
              <Star className="size-3 fill-amber-300" />
              {anime.score.toFixed(1)}
            </span>
          ) : (
            <span />
          )}
          {tracked && (
            <span className="grid size-6 place-items-center rounded-full bg-primary text-primary-foreground shadow-lg ring-2 ring-white/20">
              <Check className="size-3.5" strokeWidth={3} />
            </span>
          )}
        </div>

        {/* rank badge */}
        {showRank && rank ? (
          <div className="absolute left-0 top-9">
            <span className="block bg-primary px-2.5 py-1 text-xs font-extrabold text-primary-foreground shadow-lg">
              #{rank}
            </span>
          </div>
        ) : null}

        {/* bottom gradient + status + progress */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/85 via-black/30 to-transparent" />
        {tracked ? (
          <div className="absolute bottom-2 left-2">
            <span className="badge-base badge-info !px-1.5 !py-0.5 !text-[10px] uppercase tracking-wide">
              {tracked.status}
            </span>
          </div>
        ) : null}
        {tracked && tracked.episodes ? (
          <div className="absolute inset-x-0 bottom-0 h-1 bg-black/50">
            <div
              className="h-full bg-gradient-to-r from-primary to-primary/70"
              style={{
                width: `${Math.min(
                  100,
                  (tracked.progress / tracked.episodes) * 100
                )}%`,
              }}
            />
          </div>
        ) : null}

        {/* hover overlay quick-info */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 translate-y-2 p-3 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <p className="line-clamp-2 text-xs text-white/90">
            {anime.synopsis ? anime.synopsis.slice(0, 90) + "…" : "Tap to view details"}
          </p>
        </div>
      </div>

      {/* Info */}
      <div className="flex flex-1 flex-col gap-1 p-2.5">
        <h3 className="card-title" title={anime.title_english || anime.title}>
          {anime.title_english || anime.title}
        </h3>
        <div className="card-meta">
          {anime.type && <span>{anime.type}</span>}
          {anime.episodes ? (
            <>
              <span aria-hidden>·</span>
              <span>{anime.episodes} ep</span>
            </>
          ) : null}
          {anime.year ? (
            <>
              <span aria-hidden>·</span>
              <span>{anime.year}</span>
            </>
          ) : null}
        </div>
      </div>
    </button>
  );
}
